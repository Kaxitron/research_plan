...
[← Taylor Expansions](lesson-21-taylor-implicit.md) | [Back to TOC](../README.md) | [Next: Fundamental Theorems of Vector Calculus →](lesson-21c-fundamental-theorems.md)
...
- **Gradient fields:** given a scalar function φ(x), the gradient ∇φ is a vector field. It points in the direction of steepest increase, with magnitude equal to the rate of increase. You already know this from Lesson 15 — now we're placing it in the broader context of all vector fields.
...
- **Key exercise:** for the "double well" potential φ(x, y) = (x² − 1)² + y², compute the gradient field, plot it, compute the divergence, and identify the critical points. Classify each critical point using the Hessian (connecting to Lesson 21 and Phase 1 eigenvalue analysis).
...
- **Forward to PDEs:** the Laplacian IS the heat equation (Lesson 27). Understanding it here means the PDE lesson can focus on solutions rather than introducing the operator.
...
- **Forward to Phase 5 topology:** the exact sequence grad → curl → div (where each composition is zero) is a de Rham complex. It connects the differential operators of this lesson to the topological invariants of Lesson 58.
