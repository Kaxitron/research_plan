...
[← Transformer](lesson-47-transformer.md) | [Back to TOC](../README.md) | [Next: LLM Pipeline →](lesson-49-llm-pipeline.md)
...
- **The RLHF pipeline:**
  1. **Pre-train** a language model on text (next-token prediction / MLE — Lesson 17)
  ...
  5. **KL penalty:** don't let the model drift too far from the SFT model (prevent reward hacking by staying near known-good behavior). This is the Bayesian/constrained optimization connection from Lessons 28 and 23.
...
- **Reward hacking** is the practical manifestation of Goodhart's Law and specification gaming (Lesson 51).
- **The reward model** is the proxy for human values. Its imperfections are the source of misalignment.
- **PPO's trust region** is constrained optimization (Lesson 28) — don't change too much at once.
- **The KL penalty** is Bayesian regularization (Lesson 21) — the base model is the prior.
- **Scalable oversight** (Lesson 52) asks: what happens when the agent is too smart for the reward model to evaluate?