# Lesson 53: Interpretability — Singular Learning Theory — The Geometry of Learning

[← Interp Scaling](lesson-52-interp-scaling.md) | [Back to TOC](../README.md) | [Next: Turing Machines →](../phase5-math-enrichment/lesson-54-turing-machines.md)
...
- **The problem:** classical statistics (Lesson 27's bias-variance tradeoff) assumes the model is *regular* — meaning the mapping from parameters to predictions is smooth and one-to-one near the optimal solution. The Fisher information matrix is invertible. The loss landscape is locally quadratic (bowl-shaped).
...
- **Phase transitions in training** are moments when the model suddenly learns a new structure — the loss drops sharply, and the internal representations reorganize. Grokking (Lesson 29) is an example.
...
- **Understanding capability emergence:** if capabilities correspond to phase transitions between singularities, SLT gives a mathematical framework for predicting *when* and *how* capabilities emerge. This is the mathematical foundation for the scaling/emergence questions in Lesson 46.
...
- **Key exercise:** Take the grokking demo from Lesson 29. At the moment of grokking (sudden generalization), what happens to the internal structure? How would you interpret this as a phase transition between singularities?
