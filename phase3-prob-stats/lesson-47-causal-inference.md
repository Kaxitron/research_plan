...
[← Bayesian Model Comparison](lesson-46-bayesian-model-comparison.md) | [Back to TOC](../README.md) | [Next: Applied Statistics →](lesson-48-applied-statistics.md)

---

> **Why this lesson exists:** This is arguably the most important lesson in the entire statistics phase. Causal inference is what separates "we observed a correlation" from "X actually causes Y." The techniques here — DAGs, counterfactuals, instrumental variables — give you a rigorous framework for evaluating causal claims. This is directly relevant to alignment (counterfactual reasoning is central to decision theory, Lesson 49) and to the genetics/epidemiology debates you want to adjudicate.
...
1. **Brady Neal — "Introduction to Causal Inference" (YouTube course)**
   ...
   - *A full course, but the first 3-5 lectures cover all the core concepts. The best free resource on the topic.*
...
- **Counterfactual reasoning** is central to interpretability. "If this neuron had fired differently, would the output change?" Activation patching (Lesson 44) IS causal inference — you're intervening on variables and measuring effects.
...
- **Decision theory (Lesson 49) IS causal reasoning.** CDT uses causal interventions ("if I do X, what happens?"). EDT uses correlations. FDT uses logical/functional connections. The CDT vs. EDT debate is literally about whether agents should reason causally or evidentially.
