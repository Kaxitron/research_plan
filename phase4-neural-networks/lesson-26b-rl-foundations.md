# Lesson 26b: Reinforcement Learning Foundations

[← Transformer](lesson-26-transformer.md) | [Back to TOC](../README.md) | [Next: The LLM Training Pipeline →](lesson-26c-llm-pipeline.md)
...
- **The RLHF pipeline:**
  1. **Pre-train** a language model on text (next-token prediction / MLE — Lesson 19)
  ...
  5. **KL penalty:** don't let the model drift too far from the SFT model (prevent reward hacking by staying near known-good behavior). This is the Bayesian/constrained optimization connection from Lessons 15b and 21.
...
- **GridWorld policy gradient:** Implement REINFORCE on a simple 5×5 grid. Agent starts at corner, goal at opposite corner, -1 reward per step. Watch the policy converge from random to shortest-path.
...
- **Reward hacking** is the practical manifestation of Goodhart's Law and specification gaming (Lesson 32).
- **The reward model** is the proxy for human values. Its imperfections are the source of misalignment.
- **PPO's trust region** is constrained optimization (Lesson 15b) — don't change too much at once.
- **The KL penalty** is Bayesian regularization (Lesson 21) — the base model is the prior.
- **Scalable oversight** (Lesson 33) asks: what happens when the agent is too smart for the reward model to evaluate?
