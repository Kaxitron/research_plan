# Lesson 21d: Regression — From Linear to Logistic and Beyond

[← Experimental Design](lesson-21c-experimental-design.md) | [Back to TOC](../README.md) | [Next: Causal Inference →](lesson-21e-causal-inference.md)

---

> **Why this lesson exists:** You already understand linear regression geometrically — it's projection onto a subspace (Lesson 9). Now you need regression as a *statistical* tool: what do coefficients mean, when can you trust them, what happens with multiple predictors, and how do you handle binary outcomes (logistic regression). Most real-world statistical claims — in genetics, epidemiology, economics, and ML — come from regression models. Reading these results fluently is a prerequisite for adjudicating debates.
...
- **T-tests on coefficients:** "is β₁ significantly different from zero?" This uses the same hypothesis testing framework from Lesson 21b. The test statistic is t = β̂₁ / SE(β̂₁).
...
| **p-value** | Significance of this coefficient (see Lesson 21b for caveats) |
...
- **The logistic function:** σ(z) = 1/(1 + e⁻ᶻ) — maps any number to (0, 1). You already know this as the sigmoid from neural networks (Lesson 22).
...
- **The pattern:** y = f(β₀ + β₁x₁ + ...) where f is a "link function" that maps the linear prediction to the appropriate range.
  ...
  - Logistic regression: f = logistic (maps to 0–1)
...
- **"Statistical Rethinking" by McElreath** — Chapters 4–5 for the Bayesian take on regression
